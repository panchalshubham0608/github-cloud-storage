...
import 'jest';
import { GetMetadataResponse } from '../response';
...
describe('BlobReader', () => {
    ...
    // create a new client - retry is disabled for testing
    const client: IClient = new Client({owner, repository, token, retry: false});
    ...
    it('BlobReader should retrieve correct owner name', () => {
        expect(blobReader.OwnerName()).toBe(owner);
    });

    describe('BlobReader GetMetadata', () => {
        it('should retrieve metadata for public repository without authorization', async () => {
            const client = new Client({owner, repository: 'panchalshubham0608', token: ''});
            const blobReader = client.NewBlobReader();
            let error = null;
            let metadataResponse : GetMetadataResponse | null = null;
            try {
                metadataResponse = await blobReader.GetMetadata('README.md');
            } catch (err) {
                error = err;
            }
            ...
            expect(metadataResponse?.metadata).not.toBeNull();
        });

        it('should throw error ErrKindUnauthorized when authorization token is not correct', async () => {
        ...
        it('should throw error ErrKindNotFound when blob does not exist', async () => {
            let error = null;
            try {
        ...
        it('should throw error ErrKindUnprocessableEntity when blob is not a file', async () => {
            let error = null;
            try {
        ...
        it('should return correct metadata for a file', async () => {
            ...
            let error = null;
            try {
                metadataResponse = await blobReader.GetMetadata(path);
            } catch(err) {
                error = err;
            }
            expect(error).toBeNull();
            ...
            let metadata = metadataResponse?.metadata;
            ...
            expect(metadata?.etag).toContain(etag);
            expect(metadata).toHaveProperty('last_modified');
            expect(metadata?.last_modified).toBeInstanceOf(Date);
        });

        it('should not throw error for a file that contains array as content', async () => {
            const path = 'files/file3.txt';
            let metadataResponse : GetMetadataResponse | null = null;
            let error = null;
            try {
                metadataResponse = await blobReader.GetMetadata(path);
